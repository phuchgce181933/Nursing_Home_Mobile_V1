      ...
      <Portal>
        <Dialog visible={showForm} onDismiss={() => setShowForm(false)} style={{ borderRadius: 16 }}>
            ...
            <View style={styles.chipRow}>
              {BLOOD_TYPES.map((bt) => (
                <Chip key={bt} selected={bloodType === bt} onPress={() => setBloodType(bt)}
                  style={bloodType === bt ? { backgroundColor: roleColor } : undefined}
          ...
          <Dialog.Actions>
            ...
            <Button mode="contained" buttonColor={roleColor} onPress={handleSubmit} loading={saveMutation.isPending}>{t(`${NS}.save`)}</Button>
          </Dialog.Actions>
        </Dialog>
      </Portal>
    </View>
...
const createStyles = (c: AppColors) => StyleSheet.create({
  ...
  chipLabel: { fontSize: 11, color: c.textSecondary, marginBottom: 6 },
  chipRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 12 },
  charCount: { color: c.textMuted, fontSize: 10, textAlign: 'right', marginBottom: 4 },
