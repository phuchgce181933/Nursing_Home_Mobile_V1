              ...
              <Card.Content>
                <View style={styles.row}>
                  <View style={{ flex: 1 }}>
                    ...
                    {item.daysRequested ? <Text style={[styles.daysCount, { color: roleColor }]}>{t(`${NS}.daysCount`, { count: item.daysRequested })}</Text> : null}
      ...
      <Portal>
        <Dialog visible={showCreate} onDismiss={() => setShowCreate(false)} style={{ borderRadius: 16 }}>
          ...
          <Dialog.ScrollArea style={{ maxHeight: 480 }}>
            ...
            <View style={styles.chipRow2}>
              {LEAVE_TYPES.map(lt => (
                <Chip key={lt.value} selected={form.type === lt.value}
                  ...
                  style={form.type === lt.value ? { backgroundColor: roleColor } : undefined}
            ...
            {daysCount > 0 && (
              <View style={styles.daysPreview}>
                <Text style={[styles.daysPreviewText, { color: roleColor }]}>
          ...
          <Dialog.Actions>
            ...
            <Button mode="contained" buttonColor={roleColor} onPress={handleCreate} loading={createMut.isPending}>{t(`${NS}.submit`)}</Button>
          </Dialog.Actions>
        </Dialog>
...
const createStyles = (c: AppColors) => StyleSheet.create({
  ...
  daysCount: { fontSize: 11, marginTop: 2 },
