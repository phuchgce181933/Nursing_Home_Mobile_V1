  ...
  return (
    <View style={styles.flex}>
      ...
      <Portal>
            ...
            <Dialog.ScrollArea style={{ maxHeight: 350 }}>
              <ScrollView keyboardShouldPersistTaps="handled">
                <TextInput
                  ...
                  returnKeyType="next" submitBehavior="submit" onSubmitEditing={() => ageRef.current?.focus()}
                />
                <TextInput
                  ref={ageRef}
                  label={t(`${NS}.ageInputLabel`)} mode="outlined" value={form.age}
                  onChangeText={v => setForm(f => ({ ...f, age: v.replace(/[^0-9]/g, '') }))} dense keyboardType="numeric" style={styles.input} error={ageInvalid}
                ...
                <TextInput
                  ref={phoneRef}
                  label={t(`${NS}.phoneLabel`)} mode="outlined" value={form.phone}
                  onChangeText={v => setForm(f => ({ ...f, phone: v }))} dense keyboardType="phone-pad" style={styles.input}
                  returnKeyType="next" submitBehavior="submit" onSubmitEditing={() => addressRef.current?.focus()}
                />
                <TextInput
                  ref={addressRef}
                  label={t(`${NS}.addressLabel`)} mode="outlined" value={form.address}
                  onChangeText={v => setForm(f => ({ ...f, address: v }))} dense style={styles.input}
        ...
        <Dialog visible={!!closeId} onDismiss={() => setCloseId(null)} dismissable={false} dismissableBackButton>
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
            <Dialog.Title>{t(`${NS}.closeConfirmTitle`)}</Dialog.Title>
            <Dialog.Content>
              <TextInput label={t(`${NS}.closeNoteLabel`)} mode="outlined" value={closeNote} onChangeText={setCloseNote} dense multiline returnKeyType="done" submitBehavior="blurAndSubmit" />
            </Dialog.Content>
