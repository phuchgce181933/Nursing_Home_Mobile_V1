...
export default function CaregiverProfile() {
  ...
  const initials = (user?.fullName ?? 'CS').split(' ').map((w: string) => w[0]).slice(-2).join('').toUpperCase();
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <AnimCard delay={0}>
        <View style={styles.profileHeader}>
          ...
          <ThemedText style={styles.name}>{user?.fullName ?? 'Người chăm sóc'}</ThemedText>
          <ThemedText style={styles.email}>{user?.email ?? ''}</ThemedText>
          <View style={styles.roleBadge}>
            <ThemedText style={styles.roleText}>🤲 Người chăm sóc</ThemedText>
          </View>
          <View style={styles.statsRow}>
            ...
            <View style={styles.statItem}>
              <ThemedText style={styles.statVal}>7</ThemedText>
              <ThemedText style={styles.statLabel}>Nhiệm vụ</ThemedText>
            </View>
            <View style={styles.statDivider} />
            <View style={styles.statItem}>
              <ThemedText style={styles.statVal}>2</ThemedText>
      ...
      <AnimCard delay={100}>
        <View style={styles.section}>
          ...
          <View style={styles.menuGroup}>
            <MenuItem icon="👤" label="Thông tin cá nhân" sub="Họ tên, kinh nghiệm, chứng chỉ" />
      ...
      <AnimCard delay={180}>
        <View style={styles.section}>
          ...
          <View style={styles.menuGroup}>
            <MenuItem icon="📋" label="Lịch làm việc" sub="Xem lịch ca, ngày nghỉ" />
            <MenuItem icon="📊" label="Báo cáo nhiệm vụ" sub="Tóm tắt hoàn thành nhiệm vụ" />
            <MenuItem icon="🏆" label="Thành tích" sub="Xem kết quả đánh giá" />
          </View>
        </View>
      </AnimCard>
...
const styles = StyleSheet.create({
  ...
  profileHeader: { backgroundColor: Colors.caregiver, borderRadius: Radius.xl, padding: Spacing.five, alignItems: 'center', ...Shadow.md },
  ...
  menuIcon: { width: 40, height: 40, borderRadius: Radius.md, backgroundColor: '#FFF3E0', justifyContent: 'center', alignItems: 'center' },
